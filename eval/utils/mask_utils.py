    ...
    def run(self, rgb):
        ...
        while not self.done:
            ...
            if self.mask is not None:
                ...
                tmp = vis_mask(tmp, mask_to_show.astype(np.uint8), color=[0, 255, 0], alpha=0.5).astype(
                    np.uint8)
...
def create_mask(image):
    drawer = PromptDrawer(screen_scale=3.0, sam_checkpoint="/home/yulin/Documents/yulin/cam_robot_calib/crc/third_party/segment_anything/sam_vit_h_4b8939.pth")
    _, _, mask = drawer.run(image)
    if mask is not None:
        out_path = "mask.png"
