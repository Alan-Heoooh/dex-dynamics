...
class RealAbilityHand:
    ...
    #     # index, middle, ring, pinky, thumb q2, thumb q1
    #     # map from below to above
    #     #10.27the order in sim hand action is: thumb q1/index_q1/middle_q1/ring_q1/pinky_q1/thumb q2
    ...
    # def joint_remap_6_qpos_to_10(joint):
        pass
        # Should map from [index, middle, ring, pinky, thumb q2, thumb q1]
        # to [thumb q1/index_q1/middle_q1/ring_q1/pinky_q1/thumb q2]
        ...
        #     augmented_joint_pos_read += [
        #         current_temp_read,
        #         1.05851325*current_temp_read+0.72349796,
        #     ]
        # return np.array(augmented_joint_pos_read)

    def joint_sim_to_hardware(self, joint_angle):
        # Map from [thumb q1/index_q1/middle_q1/ring_q1/pinky_q1/thumb q2]
        # to [index, middle, ring, pinky, thumb q2, thumb q1]
        return joint_angle[[1, 2, 3, 4, 5, 0]]

    def joint_hardware_to_sim(self, joint_angle):
        # Map from [index, middle, ring, pinky, thumb q2, thumb q1]
        # to [thumb q1/index_q1/middle_q1/ring_q1/pinky_q1/thumb q2]
        return joint_angle[[5, 0, 1, 2, 3, 4]]

    def set_joint_angle(self, joint_angle, reply_mode=None, dt=0.01):
        """
        The order is: thumb_q1, index_q1, middle_q1, ring_q1, pinky_q1, thumb_q2
        """
        # if len(joint_angle) == 10:
        #     joint_angle = self.joint_remap_10_qpos_to_6(joint_angle)
        joint_angle = self.joint_sim_to_hardware(joint_angle)
        ...
        if reply_mode & 0x10 == 0x10:
            with self.current_joint_target.get_lock():
                # print(self.current_joint_target.get_lock())
                # print(joint_angle)
    ...
    def get_hand_state(self): 
        ...
        with self.current_joint_pos_read.get_lock():
            ...
            current_qpos[:] = self.current_joint_pos_read[:] # angle
    ...
    def get_data(self):
        ...
        raw_pos_hardware = data["raw_pos"]
        raw_pos_sim = self.joint_hardware_to_sim(raw_pos_hardware)
        # print(data["raw_pos"]) # :6
        # print(data["vel"]) # 6:12
        # print(data["touch"]) # 12:41
        data = np.concatenate([raw_pos_sim, data["vel"], data["touch"]])
        return data
