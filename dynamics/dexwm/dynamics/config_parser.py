...
class ConfigParser:
    ...
    def update_from_yaml(self, config_json):
        # try:
        update_config = read_yaml(config_json)
        if update_config.get("config", None) is not None:
            update_config = update_config["config"]

        self._config.update(update_config)

    # except Exception as e:
    #     self._config.update(read_stored_config(config_json))
...
# def config_parser_constructor(loader, node):
#     data = loader.construct_mapping(node, deep=True)
#     return ConfigParser(**data)


# yaml.FullLoader.add_constructor(
#     "tag:yaml.org,2002:python/object:dynamics.config_parser.ConfigParser",
#     config_parser_constructor,
# )
# Custom constructor for ConfigParser
def config_parser_constructor(loader, node):
    data = loader.construct_mapping(node, deep=True)
    return data["_config"]


# Custom constructor for torch.device
...
yaml.FullLoader.add_constructor(
    "tag:yaml.org,2002:python/object/apply:torch.device", torch_device_constructor
)
