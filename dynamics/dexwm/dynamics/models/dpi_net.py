...
from .utils_dynamics import MLP, rigid_params_to_motion
...
class DPINet(MessagePassing):
    def __init__(self, config):
        ...
        # NodeEncoder
        node_feature_dim = self.F_t + self.F_a + self.P # particle type, action, position
    ...
    def forward(self, data):
        ...
        # renormalize to the object center
        pos_expand = data.pos.view(B, N, self.H, self.P)
        # object_center = torch.mean(pos_expand[:, : self.N_list[0]], dim=1, keepdim=True)
        # pos_expand = pos_expand - object_center

        # Compute offsets

        offsets = []
        obj_indices = self.N_cum.tolist()
