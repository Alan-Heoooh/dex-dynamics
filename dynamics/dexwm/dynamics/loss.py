...
class PositionLoss(torch.nn.Module):
    ...
    def __call__(self, x, y, return_losses=False):
        ...
        # [object]
        x_list = [
            x[:, self.N_cum[i] : self.N_cum[i + 1]] for i in range(len(self.N_cum) - 2)
        ]
        y_list = [
            y[:, self.N_cum[i] : self.N_cum[i + 1]] for i in range(len(self.N_cum) - 2)
        ]
        ...
        for i, (x, y, n) in enumerate(zip(x_list, y_list, self.N_list)):
            ...
            if self.loss_type == "mse":
                ...
                if self.loss_weights["emd"] > 0:
                    ...
                    emd_loss_total += emd_loss

                # print(f"object {i} chamfer_loss: {chamfer_loss}, emd_loss: {emd_loss}")
            else:
        ...
        if self.loss_type == "chamfer_emd" and return_losses:
            return loss, losses, chamfer_loss_total, emd_loss_total
