...
from dexwm.utils.utils import *
from dexwm.utils.visualizer import *
...
def filter_edges_by_distance(pos, pos_p, edge_index, distance_limit):
    """
    Filter edges based on the distance between the connected nodes.

    Parameters:
    - pos: Tensor of shape (N, 3) representing the positions of the nodes.
    - pos_p: Tensor of shape (N, 3) representing the positions of the nodes in the other object.
    - edge_index: Tensor of shape (2, E) representing the edges.
    - radius: The radius for the distance calculation.
    - distance_limit: The maximum distance between connected nodes.

    Returns:
    - Filtered edge_index.
    """
    
    # Calculate the distance between the connected nodes
    edge_displacement = pos[edge_index[1]] - pos_p[edge_index[0]]
    edge_distance = torch.norm(edge_displacement, dim=1)

    # Retain edges whose distance is smaller than the limit
    valid_edges_mask = edge_distance <= distance_limit
    filtered_edge_index = edge_index[:, valid_edges_mask]

    return filtered_edge_index

def compute_radius_unsorted_batch(
    x,
    y,
    r,
    batch_x,
    batch_y,
    max_num_neighbors,
    batch_x_presorted=False,
    batch_y_presorted=False,
):
    ...
    if batch_x_presorted:
        ...
        edge_pairs_x_unsorted = torch.arange(batch_x.shape[0], device=batch_x.device)[
            batch_x_sorted_idx
        ][edge_pairs_x]
    ...
    if batch_y_presorted:
        ...
        edge_pairs_y_unsorted = torch.arange(batch_y.shape[0], device=batch_y.device)[
            batch_y_sorted_idx
        ][edge_pairs_y]
...
def compute_knn_unsorted_batch(
    x,
    y,
    k,
    batch_x,
    ...
    """Batch-aware KNN with index remapping"""
    # Sort batches if needed
    if not batch_x_presorted:
        batch_x_sorted, batch_x_sorted_idx = torch.sort(batch_x)
        x = x[batch_x_sorted_idx]
    else:
        batch_x_sorted = batch_x
    ...
    # Remap indices to original ordering
    edge_pairs_y, edge_pairs_x = edge_pairs[0], edge_pairs[1]

    if not batch_x_presorted:
        edge_pairs_x = batch_x_sorted_idx[edge_pairs_x]

    if not batch_y_presorted:
        edge_pairs_y = batch_y_sorted_idx[edge_pairs_y]

    return torch.stack((edge_pairs_y, edge_pairs_x), dim=0)

def get_batch_vector(
    num_nodes, batch_size, node_index_cumsum, shift_obj_idx=0, device="cuda"
):
    """
    ...
    tensor += torch.arange(batch_size, device=device).view(-1, 1) * (
        len(node_index_cumsum) - 1
    )
...
def connect_edges_batched(config, pos, total_num_points, batch_size, cumul_points):
    N = total_num_points
    all_pts = pos
    all_edges = []
    all_edge_attr = []
    max_object_pts = max(config["n_points"])

    # Inner connections (within objects)
    if config["use_knn_inner"]:
        object_obs_edges = pyg.nn.knn_graph(
            all_pts.reshape(N * batch_size, 3),
            k=config["k_neighbors_inner"],
            batch=get_batch_vector(
                N, batch_size, cumul_points, device=all_pts.device
            ),
            loop=False
        )
    else:
        object_obs_edges = pyg.nn.radius_graph(
            all_pts.reshape(N * batch_size, 3),
            r=config["connectivity_radius_inner"],
            batch=get_batch_vector(
                N, batch_size, cumul_points, device=all_pts.device
            ),
    ...
    if config["filter_edges_by_distance_inner"]:
        object_obs_edges = filter_edges_by_distance(
            all_pts.reshape(N * batch_size, 3),
            all_pts.reshape(N * batch_size, 3),
            object_obs_edges,
            config["edge_distance_limit"]
        )

    all_edge_attr.append(
        get_edge_attr(
            all_pts.reshape(N * batch_size, 3),
            all_pts.reshape(N * batch_size, 3),
            object_obs_edges,
            config["connectivity_radius_inner"],
        )
    )
    ...
    for obj_comparison_idx in range(num_obj - 1):
        if config["use_knn_outer"]:
            edge_pairs = compute_knn_unsorted_batch(
                x=all_pts.reshape(N * batch_size, 3),
                y=all_pts.reshape(N * batch_size, 3),
                k=config["k_neighbors_outer"],
                batch_x=get_batch_vector(
                    N,
                    batch_size,
                    cumul_points,
                    shift_obj_idx=obj_comparison_idx + 1,
                    device=all_pts.device,
                ).long(),
                batch_y=get_batch_vector(
                    N, batch_size, cumul_points, device=all_pts.device
                ).long(),
        ...
        if config["filter_edges_by_distance_outer"]:
            edge_pairs = filter_edges_by_distance(
                all_pts.reshape(N * batch_size, 3),
                all_pts.reshape(N * batch_size, 3),
                edge_pairs,
                config["edge_distance_limit"]
    ...
    # Combine all edges and attributes
...
def compute_slice_indices(tensor, points_per_graph, batch_size):
    ...
    first_occurrence = torch.cat(
        (first_occurrence, torch.tensor([tensor.shape[0]]).to(tensor.device))
    )
...
def connect_edges(config, pos_dict):
    ...
    for key, (idx_offset, pos) in pos_dict.items():
        ...
        if config["use_knn_inner"]:
            edge_pairs = pyg.nn.knn_graph(pos, k=config["k_neighbors_inner"], loop=False)
        else:
        ...
        if config['filter_edges_by_distance_inner']:
            edge_pairs = filter_edges_by_distance(pos, pos, edge_pairs, config['edge_distance_limit'])
    ...
    for pair in pairs:
        ...
        if config['filter_edges_by_distance_outer']:
            edge_pairs = filter_edges_by_distance(pos, pos_p, edge_pairs, config['edge_distance_limit'])
        ...
        if config["debug"]:
            print(
                f"inter-object ({idx_offset}, {idx_offset_p}) connections count: {edge_pairs.shape[-1]}"
            )
...
def farthest_point_sampling_numpy(points, n_points):
    ...
    assert (
        n_points <= N
    ), "Number of points to sample cannot exceed the total number of points"
    ...
    indices = (
        farthest_point_sampler(points_tensor.unsqueeze(0), n_points).squeeze(0).numpy()
    )  # , start_idx=0 for deterministic fps
...
def get_object_pcs_from_state_dict(state_dict, config):
    ...
    for i in range(len(state_dict["object_pcs"])):
        objects.append(
            torch.from_numpy(
                preprocess_points_to_particles(
                    state_dict["object_pcs"][i], config["particles_per_obj"]
                )
            )
        )
...
def construct_graph_from_video(config, state_list, target=False, trace=False):
    ...
    in_hand_object_obs = torch.stack(
        [
            torch.from_numpy(
                preprocess_points_to_particles(
                    d["inhand_object_pcs"], config["particles_per_obj"]
                )
            )
            for d in state_list
        ]
    ...
    bubble_gt = torch.stack(
        [
            torch.from_numpy(
                preprocess_points_to_particles(
                    d["bubble_pcs"].reshape(-1, d["bubble_pcs"].shape[-1]),
                    config["particles_per_obj"],
                )
            )
            for d in state_list
        ]
    )  # view two bubbles as one
    # object class
    ...
    # tactile reading
    forces = torch.stack(
        [torch.from_numpy(d["forces"]) for d in state_list]
    )  # (T, 2, 7)
    ...
    actions = torch.cat(
        (
            torch.zeros(
                (T - 1, n_points_object + n_points_inhand, 3), dtype=torch.float32
            ),
            tool_actions,
        ),
        dim=1,
    ...
    node_features = torch.cat(
        (particle_type, rgb_features, actions), dim=-1
    )  # first dim N_o + n_points_inhand + N_t

    # construct the graph based on the distances between particles
    pos_dict = OrderedDict(
        object_obs=(
            0,
            object_obs[his_len - 1, :, :3],
    ...
    pos_seq = (
        torch.cat(
            (object_obs[..., :3], in_hand_object_obs[..., :3], bubble_gt[..., :3]),
            dim=1,
        )
        .transpose(0, 1)
        .reshape(n_points, -1)
    ...
    if config["debug"]:
        ...
        if tick > 15 and tick % 2 == 0:
            ...
            visualize_networkx(
                ...
                nx_graph,
                pos_seq,
                config["particles_per_obj"] * 2,
                f"networkx/tick_{tick}.jpg",
            )
        tick += 1
...
def downsample_points_state_dict_seq(
    state_dict_seq,
    config,
    return_mask=False,
    object_masks=None,
    inhand_object_mask=None,
):
    # return state_dict_seq
    # assert that n_points have the same value as particles_per_obj
    assert (
        np.array(config["n_points"]) == config["particles_per_obj"]
    ).all(), f"n_points should be the same as particles_per_obj, but got {config['n_points']} and {config['particles_per_obj']}"
    ...
    for i, state_dict in enumerate(state_dict_seq):
        ...
        }  # critical, we don't want inplace operations

        # process inhand object points
        if "inhand_object_pcs" in state_dict:
            if i == 0 and not preloaded_inhand_object_mask:
                state_dict["inhand_object_pcs"], inhand_object_mask = (
                    farthest_point_sampling_numpy(
                        state_dict["inhand_object_pcs"], n_points
                    )
                )
            else:
                state_dict["inhand_object_pcs"] = state_dict["inhand_object_pcs"][
                    inhand_object_mask
                ]

        # process softbubble points
        new_bubble_pcs = [[], []]
        for j, bubble_pc in enumerate(
            state_dict["bubble_pcs"]
        ):  # for each bubble out of the two
            if i == 0:
                new_bubble_pc, softbubble_mask = farthest_point_sampling_numpy(
                    bubble_pc, n_points
                )
        ...
        for j, object_pc in enumerate(state_dict["object_pcs"]):
            if i == 0 and not preloaded_object_masks:
                new_object_pc, object_mask = farthest_point_sampling_numpy(
                    object_pc, n_points
                )
        ...
        global counter
        state_dict["rand_index"] = (
            np.zeros(len(state_dict["object_pcs"]), dtype=np.int32) + counter
        )
...
def replicate_first_bubble_pcs_for_all(state_dict_seq):
    ...
    for i, state_dict in enumerate(state_dict_seq):
        ...
        if bubble_points is None:
            ...
            delta = state_dict["bubble_pcs"][..., :3].reshape(-1, 3).mean(
                0
            ) - bubble_points[..., :3].reshape(-1, 3).mean(0)
...
def shift_state_dict_seq(state_list):
    ...
    offset_t = int(
        np.round(0.2 * 8)
    )  # number of frames for latency in observations: 6 frames / sec * 0.2 secs
...
def filter_points_by_distance_to_center(
    point_cloud, distance_limit=1.0, limit_percentile=50
):
    """
...
def denoise_softbubble_points(state_list):
    ...
    for state_dict in state_list:
        ...
        denoised_bubble_points = filter_points_by_distance_to_center(
            state_dict["bubble_pcs"], distance_limit=0.5
        )
...
class OneStepDataset(pyg.data.InMemoryDataset):
    ...
    def process(self):
        ...
        for vid_path in sorted(Path(self.root).glob("*")):
            ...
            state_list = replace_consecutive_failing_elements(
                state_list,
            ...
            for i in range(
                random.randint(0, self.config["every_n_frame"] - 1),
                len(state_list) - (H + W) + 1,
                self.config["every_n_frame"],
            ):
                # history frames
                ...
                state_window = downsample_points_state_dict_seq(
                    state_window, self.config
                )
                ...
                assert (
                    data.object_cls == data.object_cls[0]
                ).all(), "object_cls should be the same for all steps"
...
class RolloutDataset(pyg.data.InMemoryDataset):
    def __init__(
        ...
        all_state_list = replace_consecutive_failing_elements(
            all_state_list,
            lambda x: x["bubble_pcs"][..., 2].mean() < 0.27,
            num_consecutive=3,
        )
        ...
        for idx in range(0, len(all_state_list), self.config["every_n_frame"]):
            ...
            gt_state_seq = []
            for d in trajectory:
                bubble_gt = preprocess_points_to_particles(
                    d["bubble_pcs"].reshape(-1, d["bubble_pcs"].shape[-1]),
                    config["particles_per_obj"],
                )
                flows = bubble_gt[:, -2:]
                ...
                inhand = preprocess_points_to_particles(
                    d["inhand_object_pcs"], config["particles_per_obj"]
                )
                bubble = np.concatenate(
                    (
                        bubble_gt,
                        np.tile([1.0, 0.0, 0.0], (bubble_gt.shape[0], 1)),
                    ),
                    axis=-1,
                )
    ...
    def process(self):
        ...
        for idx in range(0, len(all_state_list), self.config["every_n_frame"]):
            ...
            data = construct_graph_from_video(
                self.config, trajectory, target=True, trace=False
            )
            data_list.append(data)
