...
from dexwm.utils.macros import *
    ...
    for idx, (key, value) in enumerate(state.items()):
        ...
        if key in config["pc_visible"]:
            if obj_color is not None and key == "object_obs":
                ...
                points_color = (
                    config["pc_color"][key][0] if key in config["pc_color"] else "b"
                )
...
def visualize_object_class_pred(config, batch_pred, path=None):
    ...
    def plot_one_frame(step):
        ...
        for i in range(R):
            ...
            for j in range(C):
                ...
                ax.set_title(
                    "GT: " + legend_labels[int(states[step]["object_cls"])],
                    fontweight="semibold",
                ...
                obj_color = (
                    object_color_dict[predictions[step - seq_len]]
                    if step - seq_len > 0
                    else "grey"
                )
                ...
                legend_handles = [
                    ...
                    for color, label in zip(object_color_dict.values(), legend_labels)
                ]
                legend = ax.legend(
                    handles=legend_handles, loc="upper left", fontsize="small"
                )
    ...
    if path:
        fps = 1
        play_and_save_video(
            images, output_file=str(path) + ".mp4", fps=fps, pause_time_last_frame=0
        )
        images[0].save(
            str(path) + ".gif",
            save_all=True,
    ...
    def plot_one_frame(step):
        ...
        for i in range(R):
            ...
            for j in range(C):
                ...
                object_color_dict = {0: "gold", 1: "turquoise", 2: "slateblue"}
...
def visualize_pred_gt_pos(
    config, title_list, pred_gt_pos_seqs, target=None, path=None, num_skip_frames=0
):
    print(f"Plotting... skip the first {num_skip_frames} frames of predictions")
        ...
        for i in range(R):
            ...
            for j in range(C):
                ...
                if i == 0 and step < num_skip_frames:  # skip render prediction
                    ...
                    output = visualize_points(
                        config, ax, pred_gt_pos_seqs[i][step], target_cur, True
                    )
...
def visualize_pred_gt_pos_simple(
    config,
    title_list,
    pred_gt_pos_seqs,
    target=None,
    ...
    views = (
        config["views"][:1]
        if not multiview
        else [config["views"][0], config["views"][2]]
    )  # only use the first and the third view for multivew vis
    C = len(views)
            ...
            for j in range(C):
                ...
                if i == 0:
                    if step < num_skip_frames:  # skip render prediction
                        visualize_points(
                            config, ax, pred_gt_pos_seqs[i][step], target_cur, False
                        )
                    else:
                        visualize_points(
                            config, ax, pred_gt_pos_seqs[i][step], target_cur, True
    ...
    if path:
        fps = 2
        play_and_save_video(
            images, output_file=str(path) + ".mp4", fps=fps, pause_time_last_frame=1
        )
...
def visualize_networkx(config, nx_graph, point_cloud, tool_offset, path=""):
    ...
    for edge in nx_graph.edges:
        ...
        if (
            (edge[0] < 20 and edge[1] < 20)
            or (20 < edge[0] < 40 and 20 < edge[1] < 40)
            or (edge[0] > 40 and edge[1] > 40)
        ):
            continue
        ...
        ax.plot(
            ...
            [p1[2], p2[2]],
            c=c,
            linewidth=config["point_size"] / 20,
            alpha=0.5,
        )
...
def play_and_save_video(images, output_file, fps, pause_time_last_frame=3):
    ...
    for _ in range(int(pause_time_last_frame * fps)):
        writer.append_data(
            last_frame
        )  # Write the last frame multiple times for the desired pause
