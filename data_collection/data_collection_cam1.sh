...
SERIAL_NUMBER=${SERIAL_NUMBER:-"337322071340"}              # Connect to the camera with serial number
EXIT_IF_CAMERA_NOT_FOUND=${EXIT_IF_CAMERA_NOT_FOUND:-true} # true: exit if not found; false: keep running if not found
CAMERA_NAME=${CAMERA_NAME:-"cam_1"}                      # Set the ROS2 topic name
USB_PORT=${USB_PORT:-4-1.2}         # USB port, found in realsense-viewer
# /sys/devices/pci0000:00/0000:00:1c.4/0000:06:00.0/usb4/4-1/4-1:1.0/video4linux/video0
# /sys/devices/pci0000:00/0000:00:14.0/usb4/4-1/4-1.2/4-1.2:1.0/video4linux/video14
die (){
  echo "$@" 1>&2
  exit 1
}
